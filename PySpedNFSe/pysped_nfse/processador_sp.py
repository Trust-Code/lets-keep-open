...
class ProcessadorNFSeSP(ProcessadorBase):
    ...
    def _gerar_xml_envio(self, cabecalho, lote_rps):
        ...
        for rps in lote_rps:
            ...
            if rps.get('TipoInscricaoTomador') == 'F':
                cpf_cnpj = tpCPFCNPJ(CPF=rps.get('CPFCNPJTomador'))
            elif rps.get('TipoInscricaoTomador') == 'J':
                cpf_cnpj = tpCPFCNPJ(CNPJ=rps.get('CPFCNPJTomador'))

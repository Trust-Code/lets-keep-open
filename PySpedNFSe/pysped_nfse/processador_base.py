...
class ProcessadorBase(object):
    ...
    def change_special_character(self, data):

        new_data = unicodedata.normalize('NFKD', data).encode('ASCII','ignore')

        for esp_char in SPECIAL_CHARACTER:
            new_data = new_data.replace(esp_char[0], esp_char[1])

# Embedded file name: /opt/openerp/producao/addons-extension/l10n_br_account_ext/account_move_line.py
...
class account_move_line(osv.osv):
    ...
    def search(self, cr, uid, args, offset = 0, limit = None, order = None, context = None, count = False):
        ...
        if context.get('origin') == 'account.chart':
            ...
            account_chart_rec = account_chart_obj.read(cr, uid, [last_id], [], context=context)[0]
    ...
    def create_analytic_lines(self, cr, uid, ids, context = None):
        if context is None:
            context = {}
        ret = super(account_move_line, self).create_analytic_lines(cr, uid, ids, context=context)
        analytic_line_obj = self.pool.get('account.analytic.line')
        for move_line_rec in self.browse(cr, uid, ids, context=context):
            analytic_line_src = analytic_line_obj.search(cr, uid, [('move_id', '=', move_line_rec.id)])

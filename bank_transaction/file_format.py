# Embedded file name: /opt/openerp/producao/addons-extra/bank_transaction/file_format.py
import time
import netsvc
...
class RecordFormat(osv.osv):
    ...
    def _get_id(self, cr, uid, ids, field_name, arg, context = None):
        ...
        for recordformat in self.browse(cr, uid, ids):
            src_cnab_field_format = obj_cnab_field_format.search(cr, uid, [('type', '=', 'IdType'), ('record_id', '=', recordformat.id)], context=None)
            br_cnab_field_format = obj_cnab_field_format.browse(cr, uid, src_cnab_field_format[0], context=None)
            res[recordformat.id] = br_cnab_field_format.value

        return res

    _columns = {'file_id': fields.many2one('cnab.file_format', 'File'),
...
class FieldFormat(osv.osv):
    ...
    _columns = {'record_id': fields.many2one('cnab.record_format', 'Record'),

...
class CNABImporter(osv.osv_memory):
    ...
    def generate_payment_with_journal(self, cr, uid, journal_id, bank_account_id, partner_id, amount_payment_slip, amount, entry_name, date, should_validate, context):
        """
        ...
        if voucher_vals['amount'] > amount_payment_slip:
            br_res_partner_bank = self.pool.get('res.partner.bank').browse(cr, uid, bank_account_id)
            voucher_vals.update({'payment_option': br_res_partner_bank.payment_option_default,
        ...
        onchange_voucher = voucher_obj.onchange_partner_id(cr, uid, [], partner_id=partner_id, journal_id=journal.id, amount=amount, currency_id=journal.company_id.currency_id.id, ttype='receipt', date=date, context=context)['value']
        matching_line = {}
        if onchange_voucher.get('line_cr_ids'):
            ...
            matching_ids = [ line.id for line in move_line_obj.browse(cr, uid, line_ids, context=context) if line.ref == entry_name ]
            ...
            if matching_lines:
                ...
                matching_line.update({'amount': amount,
                 'voucher_id': voucher_id})
                amount_pay = matching_line['amount']
                src_voucher_line = voucher_line_obj.search(cr, uid, [('voucher_id', '=', voucher_id), ('reconcile', '=', True)])
                if src_voucher_line:
                    amount_voucher_lines = 0
                    for rec_voucher_line in src_voucher_line:
                        _w_amount = voucher_line_obj.browse(cr, uid, rec_voucher_line).amount
    ...
    def open_bank_file(self, cr, uid, ids, context = None):
        ...
        tree_res = ir_model_data.get_object_reference(cr, uid, 'bank_transaction', 'bank_file_tree')
        tree_id = tree_res and tree_res[1] or False
        return {'name': _('Consult Bank File'),
         'view_type': 'form',
         'view_mode': 'tree',
    ...
    def open_bank_statement(self, cr, uid, ids, context = None):
        ir_model_data = self.pool.get('ir.model.data')
        form_res = ir_model_data.get_object_reference(cr, uid, 'account', 'view_bank_statement_form')
        form_id = form_res and form_res[1] or False
        tree_res = ir_model_data.get_object_reference(cr, uid, 'account', 'view_bank_statement_tree')
        tree_id = tree_res and tree_res[1] or False
        return {'name': _('Consult Bank Statement'),
         'view_type': 'form',
         'view_mode': 'tree, form',
    ...
    def _pay_invoice(self, cr, uid, journal, bank_account_id, rec_account_move_line, payment_date, payment_amount):
        ...
        self.generate_payment_with_journal(cr, uid, journal.id, bank_account_id, rec_account_move_line.partner_id.id, rec_account_move_line.debit, payment_amount, rec_account_move_line.move_id.ref, payment_date, True, context={'invoice_id': rec_account_move_line.id})
    ...
    def import_file(self, cr, uid, ids, context = None):
        ...
        if wizard.type == 'retorno':
            ...
            for line in payments:
                if obj_account_move_line.search(cr, uid, [('name', '=', line['NDocumento'])]):
                    src_account_move_line = obj_account_move_line.search(cr, uid, [('name', '=', line['NDocumento'])])
                    rec_account_move_line = obj_account_move_line.browse(cr, uid, src_account_move_line[0])
                    for events in wizard.payment_type.bank_account.bank.code_occurrence_ids:
                        if events.code_occurrence == line['IdentificacaoOcorrencia']:
                            if events.action == 'pay':
                                ...
                                obj_account_move_line.write(cr, uid, rec_account_move_line.id, {'status_aux': 'confirm',
            ...
            if wizard.payment_type.layout_retorno.filename:
                ...
                filename = 'RET_%s_%04d%02d%02d_%02d%02d%02d.txt' % (wizard.payment_type.bank_account.bank.bic,
                 now.year,
                 now.month,
                 now.day,
                 now.hour,
                 now.minute,
                 now.second)
            obj_bank_file.create(cr, uid, {'cnab_file': wizard.file,
             'filename': filename,
             'file_type': 'retorno',
             'date_time': now_utc,
        ...
        for line in payments:
            if line['DataLancamento'] != memory_date:
                _w_date = datetime.strptime(line['DataLancamento'], '%d%m%Y').date()
                period = '%s/%s' % (_w_date.strftime('%m'), _w_date.strftime('%Y'))
                scr_account_period = obj_account_period.search(cr, uid, [('name', '=', period)])
                reference = '%s - %s' % (journal.name, _w_date)
                vals = {'name': reference,
                 'date': _w_date,
                 'journal_id': journal.id,
                 'period_id': scr_account_period[0]}
                obj_account_bank_statement.create(cr, uid, vals, context=context)
                memory_date = line['DataLancamento']
            if line['DataLancamento'] == memory_date:
                date_release = datetime.strptime(line['DataLancamento'], '%d%m%Y').date()
                name_item = line['Historico']
                amount = self._cents_to_euros(line['ValorLancamento'])
                scr_account_bank_statement = obj_account_bank_statement.search(cr, uid, [('name', '=', reference)])
                if line['TipoLancamento'] == 'D':
                    if wizard.payment_type.company_id.partner_default_account_payable_id.id:
                        ...
                        raise osv.except_osv('Error', 'Not Found "Account Payment Default"')
                    ...
                    if wizard.payment_type.company_id.partner_default_account_receivable_id.id:
                        ...
                        raise osv.except_osv('Error', 'Not Found "Account Receive Default"')
                items = {'date': date_release,
                 'name': name_item,
                 'amount': amount,
                 'statement_id': scr_account_bank_statement[0],
                 'account_id': account_id}
                obj_account_bank_statement_line.create(cr, uid, items, context=context)

        return self.open_bank_statement(cr, uid, ids, context=context)

    _columns = {'file': fields.binary('File', required=True),
     'type': fields.selection([('retorno', 'Arquivo de Retorno Banc\xc3\xa1rio'), ('extrato', 'Arquivo de Extrato Banc\xc3\xa1rio')], 'File Type', required=True),

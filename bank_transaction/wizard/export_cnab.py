...
class CNABExporter(osv.osv_memory):
    ...
    def header(self, cr, uid, rec_payment_type):
        return {'IDReg': '0',
         ...
         'NomeEmpresa': self._spe_char_remove(rec_payment_type.company_id.legal_name),
    ...
    def detalhe(self, cr, uid, rec_account_move_line):
        ...
        if rec_account_move_line.company_id.partner_id.is_company == True:
            company_cnpj_cpf = '02'
        else:
            company_cnpj_cpf = '01'
        if rec_account_move_line.partner_id.is_company == True:
            partner_cnpj_cpf = '02'
        else:
            partner_cnpj_cpf = '01'
        ...
        return {'IDReg': '1',
         ...
         'NInscricaoSacado': self._only_digits(rec_account_move_line.partner_id.cnpj_cpf),
         'NomeSacado': self._spe_char_remove(rec_account_move_line.partner_id.legal_name),
         'EnderecoCompleto40': self._get_address(rec_account_move_line.partner_id, 40),
         'Bairro': self._spe_char_remove(rec_account_move_line.partner_id.district),
         'Cep': self._only_digits(rec_account_move_line.partner_id.zip),
         'Cidade': self._spe_char_remove(rec_account_move_line.partner_id.l10n_br_city_id.name),
    ...
    def generate_remessa(self, cr, uid, ids, context = None):
        ...
        br_bank_file = obj_bank_file.browse(cr, uid, context['active_ids'], context=context)
        for rec_bank_file in br_bank_file:
            w_id = rec_bank_file.id

        obj_payment_type = self.pool.get('payment.type')
        src_payment_type = obj_payment_type.search(cr, uid, [('layout_remessa', '!=', False)])
        br_payment_type = obj_payment_type.browse(cr, uid, src_payment_type, context=context)
        cont_title_banks = 0
        for rec_payment_type in br_payment_type:
            ...
            for rec_account_move_line in br_account_move_line:
                if rec_account_move_line.payment_type.code == rec_payment_type.code:
                    if file_remessa == []:
                        file_remessa.append(self.header(cr, uid, rec_payment_type))
                    file_remessa.append(self.detalhe(cr, uid, rec_account_move_line))
            ...
            if rec_payment_type.layout_remessa.filename:
                ...
                filename = 'REM_%s_%04d%02d%02d_%02d%02d%02d.txt' % (rec_payment_type.bank_account.bank.bic,
                 now.year,
                 now.month,
                 now.day,
                 now.hour,
                 now.minute,
                 now.second)
            result = self.serialize(cr, uid, rec_payment_type.layout_remessa, file_remessa)
            encoded_result = base64.b64encode(result)
            obj_bank_file.create(cr, uid, {'cnab_file': encoded_result,
             'filename': filename,
             'file_type': 'remessa',
             'date_time': now_utc,

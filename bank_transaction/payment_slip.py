    ...
    def get_file(self, cr, uid, ids, field_names, args, context = None):
        if not ids:
            ...
            for record in self.read(cr, uid, ids, ['res_id'], context=context):
                res[record['id']] = False
                src_ir_attachment = obj_ir_attachment.search(cr, uid, [('res_model', '=', 'account.move.line'), ('res_id', '=', record['id'])])
                if src_ir_attachment:
                    file_data = obj_ir_attachment._data_get(cr, uid, src_ir_attachment, field_names, args, context=None)
                    if file_data:
                        res[record['id']] = file_data[src_ir_attachment[0]]

            return res

    _columns = {'status': fields.function(status_default, type='char', string='Status'),
     'status_aux': fields.char('Status Auxiliary', size=128),
     'slip_file': fields.function(get_file, type='binary', string='Slip File'),
...
class res_partner_bank(osv.osv):
    ...
    _columns = {'bank_col_service': fields.char('Bank Collection Service', size=3),
     ...
     'payment_option_default': fields.selection([('without_writeoff', 'Keep Open'), ('with_writeoff', 'Reconcile Payment Balance')], 'Payment Difference Default'),
     'writeoff_acc_id_default': fields.many2one('account.account', 'Counterpart Account Default')}
    _defaults = {}
...
class payment_slip(osv.osv):
    ...
    def slip(self, cr, uid, ids, context = None):
        ...
        br_account_move_line = obj_account_move_line.browse(cr, uid, context['active_ids'], context=context)
        for rec_account_move_line in br_account_move_line:
            bank_code = rec_account_move_line.payment_type.bank_account.bank.bic
            if bank_code == '341':
                self.gerar_boleto_itau(cr, uid, rec_account_move_line, context=None)
            elif bank_code == '033':
                self.gerar_boleto_santander(cr, uid, rec_account_move_line, context=None)
    ...
    def gerar_boleto_itau(self, cr, uid, rec_account_move_line, context = None):
        ...
        if rec_account_move_line.payment_type.type_banking_billing == 'REG' or rec_account_move_line.payment_type.type_banking_billing == 'SRG':
            our_number = self.pool.get('ir.sequence').get(cr, uid, 'payment_slip_our_number_itau')
            obj_account_move_line.write(cr, uid, rec_account_move_line.id, {'our_number': our_number}, context=context)
        if rec_account_move_line.our_number == False:
            raise osv.except_osv(_('Error'), _('Error! Confirmation file has not validated by the bank!'))
        instructions = rec_account_move_line.payment_type.bank_account.instructions
        if instructions:
            ...
            instructions = instructions.replace('%(monthly_fine)s', value_fine)
            instructions = instructions.replace('%(monthly_interest)s', value_interest)
        if rec_account_move_line.partner_id.street2:
            sacado_endereco_1 = '%s, %s - %s - %s' % (rec_account_move_line.partner_id.street,
             rec_account_move_line.partner_id.number,
             rec_account_move_line.partner_id.street2,
             rec_account_move_line.partner_id.district)
        else:
            sacado_endereco_1 = '%s, %s - %s' % (rec_account_move_line.partner_id.street, rec_account_move_line.partner_id.number, rec_account_move_line.partner_id.district)
        sacado_endereco_2 = '%s - %s/%s' % (rec_account_move_line.partner_id.zip, rec_account_move_line.partner_id.city, rec_account_move_line.partner_id.state_id.code)
        cedente_endereco = '%s, %s - %s - %s/%s' % (rec_account_move_line.company_id.street,
        ...
        error = []
        if rec_account_move_line.company_id.partner_id.cnpj_cpf == False:
            error.append('CPF/CNPJ (cedente)')
        if rec_account_move_line.payment_type.bank_account.bank_col_service == False:
            error.append('Carteira Bancaria')
        if error:
            raise orm.except_orm(_('Error'), _('Missing data for generating the payment slip:\n\n%s') % error)
        lista_dados_itau = []
        ...
        obj_boleto.instrucoes = instructions or ''
        ...
        if obj_boleto.tipo_boleto == '1':
            w_file = tempfile.NamedTemporaryFile(prefix='Slip', suffix='.pdf', delete=False)
            ...
            filename = 'BOL_%s_%04d%02d%02d.pdf' % (rec_account_move_line.partner_id.name,
             due_date.year,
             due_date.month,
             due_date.day)
            w_file.seek(0)
            fl = w_file.read()
            src_ir_attachment = obj_ir_attachment.search(cr, uid, [('res_id', '=', rec_account_move_line.id)])
            if src_ir_attachment:
                attach_val = {'name': filename,
                 'datas_fname': filename,
                 'datas': base64.b64encode(fl)}
                obj_ir_attachment.write(cr, uid, src_ir_attachment, attach_val, context=context)
            else:
                ...
                pay_slip_vals = {'filename': filename,
                 'slip_create_date': date.today()}
                obj_account_move_line.write(cr, uid, rec_account_move_line.id, pay_slip_vals, context=context)
                w_file.close()

    def gerar_boleto_santander(self, cr, uid, rec_account_move_line, context = None):
        obj_ir_attachment = self.pool.get('ir.attachment')
        obj_account_move_line = self.pool.get('account.move.line')
        if rec_account_move_line.payment_type.type_banking_billing == 'REG' or rec_account_move_line.payment_type.type_banking_billing == 'SRG':
            our_number = self.pool.get('ir.sequence').get(cr, uid, 'payment_slip_our_number_itau')
            obj_account_move_line.write(cr, uid, rec_account_move_line.id, {'our_number': our_number}, context=context)
        if rec_account_move_line.our_number == False:
            raise osv.except_osv(_('Error'), _('Error! Payment slip does not yet confirmed by the bank.'))
        instructions = rec_account_move_line.payment_type.bank_account.instructions
        if instructions:
            fine = rec_account_move_line.payment_type.bank_account.monthly_fine
            interest = rec_account_move_line.payment_type.bank_account.monthly_interest
    ...
    def send_email(self, cr, uid, ids, context = None):
        ...
        br_account_move_line = obj_account_move_line.browse(cr, uid, context['active_ids'], context=context)
        for rec_account_move_line in br_account_move_line:
            attachment_ids = obj_ir_attachment.search(cr, uid, [('res_model', '=', 'account.move.line'), ('res_id', '=', rec_account_move_line.id)])
            obj_email_template = self.pool.get('email.template')
            template_id = obj_email_template.search(cr, uid, [('model', '=', 'account.move.line')])
            mail_values = obj_email_template.generate_email(cr, uid, template_id[0], rec_account_move_line.id, context=context)
            ...
            src_res_partner_child = obj_res_partner.search(cr, uid, [('is_company', '=', False), ('parent_id', '=', rec_account_move_line.partner_id.id), ('send_payment_slip', '=', True)])
            email_child = ''
            if src_res_partner_child:
                for rec_res_partner_child in obj_res_partner.browse(cr, uid, src_res_partner_child):
                    email_child += rec_res_partner_child.email and rec_res_partner_child.email + '; ' or ''

            vals = {'subject': mail_values['subject'],

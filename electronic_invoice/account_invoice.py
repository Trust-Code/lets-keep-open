# Embedded file name: /opt/openerp/homolog/addons-extra/electronic_invoice/account_invoice.py
from openerp.osv import orm, fields
import datetime

class account_invoice(orm.Model):
    _inherit = 'account.invoice'
    _columns = {'rps_code': fields.char('RPS Code', readonly=True, track_visibility='onchange'),
...
class electronic_invoice_event(orm.Model):
    ...
    _columns = {'event_date': fields.datetime('Event Date', readonly=True),
     'user_id': fields.many2one('res.users', 'User', readonly=True),
     'action': fields.selection([('send', 'Send'),
                ('cancel', 'Cancel'),
                ('check', 'Check'),

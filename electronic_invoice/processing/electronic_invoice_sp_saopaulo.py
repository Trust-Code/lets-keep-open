# Embedded file name: /opt/openerp/homolog/addons-extra/electronic_invoice/processing/electronic_invoice_sp_saopaulo.py
...
class ei_sp_saopaulo_001(osv.osv):
    ...
    def validate_send(self, cr, uid, ids, inv, context = None):
        if not inv.company_id.tributacao:
            return '%s - %s' % (u'Tipo de Tributa\xe7\xe3o n\xe3o informada', inv.company_id.name)
        ...
        for inv_line in inv.invoice_line:
            if not codigo_servico or not aliquota:
                ...
                if codigo_servico != inv_line.product_id.service_type_id.code:
                    return u'N\xe3o \xe9 permitido C\xf3digos de Servi\xe7os diferentes na mesma NF'
                if aliquota != inv_line.issqn_percent:
                    return u'N\xe3o \xe9 permitido Alquotas de ISS Diferentes na mesma NF'

        return ''

    def define_send(self, cr, uid, ids, inv, context = None):
        user_timezone = self.pool.get('res.users').browse(cr, uid, uid).tz
        date_now = datetime.datetime.now(pytz.timezone(user_timezone)).strftime('%Y-%m-%d')
        impostos = ('pis', 'cofins', 'inss', 'irpj', 'csll', 'issqn', 'iss_retido')
        valores = {x:0 for x in impostos}
        for inv_tax in inv.tax_line:
            if inv_tax.tax_code_id.domain in impostos and inv_tax.tax_retain:
                valores[inv_tax.tax_code_id.domain] += round(inv_tax.tax_retain, 2)
        ...
        lote_rps.append({'TipoRPS': 'RPS',
         ...
         'RazaoSocialTomador': unicode(self._spe_char_remove(inv.partner_id.legal_name) or '')[:75],
         'Logradouro': unicode(self._spe_char_remove(inv.partner_id.street) or '')[:50],
         'NumeroEndereco': inv.partner_id.number,
         'ComplementoEndereco': unicode(self._spe_char_remove(inv.partner_id.street2) or '')[:30],
         'Bairro': unicode(self._spe_char_remove(inv.partner_id.district) or '')[:30],
         ...
         'NumeroRPS': rps_code})
        cabecalho = {'CPFCNPJRemetente': re.sub('[^0-9]', '', inv.company_id.cnpj_cpf),
         'InscricaoMunicipalPrestador': re.sub('[^0-9]', '', inv.company_id.inscr_mun),
         'transacao': True,
         'dtInicio': date_now,
         'dtFim': date_now,
         'QtdRPS': len(lote_rps),
    ...
    def define_consult(self, cr, uid, ids, inv, context = None):
        consulta_rps = {'CPFCNPJRemetente': re.sub('[^0-9]', '', inv.company_id.cnpj_cpf),
         'InscricaoPrestador': re.sub('[^0-9]', '', inv.company_id.inscr_mun),
         'SerieRPS': int(inv.document_serie_id.code),
         'NumeroRPS': inv.rps_code,
         'Versao': self.VERSION}
        return consulta_rps

    def xml_send(self, cr, uid, ids, inv, lote_rps, cabecalho_rps, consulta_rps, cert_name, context = None):
        ...
        if datetime.datetime.now() > proc._certificado._data_fim_validade:
            return {'action': 'send',
             'ei_status': 'failed',
             'message': '%s - %s' % (u'Certificado Digital fora da validade', inv.company_id.name)}
        if not self._check_server(cr, uid, ids, proc.servidor):
            return {'action': 'send',
             'ei_status': 'failed',
             'message': u'Falha de comunica\xe7\xe3o com o servidor'}
        if inv.company_id.ei_environment == 'production':
            ...
            if success and not res.NFe:
                success, res, warnings, errors = proc.enviar_lote_rps(cabecalho=cabecalho_rps, lote_rps=lote_rps)
        else:
            success, res, warnings, errors = proc.testar_envio_lote_rps(cabecalho=cabecalho_rps, lote_rps=lote_rps)
        msg_alerta = ''
        for alerta_line in res.Alerta:
            msg_alerta += '%s-%s\n' % (str(alerta_line.Codigo or 0), alerta_line.Descricao or '')

        msg_erro = ''
        for erro_line in res.Erro:
            msg_erro += '%s-%s\n' % (str(erro_line.Codigo or 0), erro_line.Descricao or '')
        ...
        for chave_line in w_chave_nfe:
            if chave_line.ChaveNFe:
                chave_write = {'ei_code': chave_line.ChaveNFe.NumeroNFe,
        ...
        if inv.company_id.ei_environment == 'production':
            if msg_erro:
                ...
                result = {'action': 'send',
                 'ei_status': 'sent',
                 'message': '%s%s' % (msg_alerta, u'NF-e enviada com SUCESSO, aguardando confirma\xe7\xe3o')}
            else:
                result = {'action': 'send',
                 'ei_status': 'confirmed',
                 'message': '%s%s' % (msg_alerta, u'NF-e criada com SUCESSO')}
            ...
            if msg_erro:
                ...
                result = {'action': 'send',
                 'message': '%s%s' % (msg_alerta, u'Teste de envio de NF-e efetuado com SUCESSO')}
        return result
    ...
    def define_cancel(self, cr, uid, ids, inv, context = None):
        cancel_rps = {'CPFCNPJRemetente': re.sub('[^0-9]', '', inv.company_id.cnpj_cpf),
         'InscricaoPrestador': re.sub('[^0-9]', '', inv.company_id.inscr_mun),
    ...
    def xml_cancel(self, cr, uid, ids, inv, cancel_rps, cert_name, context = None):
        cert_password = str(inv.company_id.nfe_a1_password)
        proc = ProcessadorNFSeSP(cert_name, cert_password)
        if datetime.datetime.now() > proc._certificado._data_fim_validade:
        ...
        if inv.company_id.ei_environment == 'production':
            ...
            return {'action': 'cancel',
             'message': u'Servi\xe7o de Teste para Cancelamento INDISPON\xcdVEL'}
        msg_alerta = ''
        for alerta_line in res.Alerta:
            msg_alerta += '%s-%s\n' % (str(alerta_line.Codigo or 0), alerta_line.Descricao or '')
...
class electronic_invoice(osv.osv):
    ...
    def send_sp_saopaulo(self, cr, uid, ids, inv, cert_name, context = None):
        ...
        if msg:
            ...
            lote_rps, cabecalho_rps = ei_service_obj.define_send(cr, uid, ids, inv, context=None)
            consulta_rps = ei_service_obj.define_consult(cr, uid, ids, inv, context=None)
            result = ei_service_obj.xml_send(cr, uid, ids, inv, lote_rps, cabecalho_rps, consulta_rps, cert_name, context=None)
            return result
    ...
    def cancel_sp_saopaulo(self, cr, uid, ids, inv, cert_name, context = None):
        ...
        if msg:
            ...
            cancel_rps = ei_service_obj.define_cancel(cr, uid, ids, inv, context=None)
            result = ei_service_obj.xml_cancel(cr, uid, ids, inv, cancel_rps, cert_name, context=None)
            return result

    def inactivate_sp_saopaulo(self, cr, uid, ids, invoice, cert_name, context = None):
